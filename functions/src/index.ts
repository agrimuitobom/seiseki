...
export const generateQuiz = onCall(
  ...
  async (request) => {
    ...
    // 出題形式の指示。守られない事故を防ぐため、非常に強い制約として書く。
    const formatText = {
      mc: `【出題形式：選択問題のみ】${count}問すべてを4択の選択問題にしてください。
- すべての問題で type は必ず "mc" にしてください。
- 記述問題（type:"written"）は絶対に1問も出さないでください。
- 各問題の choices には必ず4つの選択肢を入れてください。`,
      written: `【出題形式：記述問題のみ】${count}問すべてを記述式（自分の言葉で答えを書く問題）にしてください。
- すべての問題で type は必ず "written" にしてください。
- 選択問題（type:"mc"）や4択は絶対に1問も出さないでください。選択肢は作らないでください。
- 各問題の choices は必ず空配列 [] にし、answer には模範解答（記述の答え）を入れてください。
- 「次のうち正しいものを選べ」のような選択式の問い方は禁止です。「〜を説明しなさい」「〜を求めなさい」「〜を書きなさい」のように記述で答える形にしてください。`,
      both: `【出題形式：選択問題と記述問題の両方】選択問題（4択, type:"mc"）と記述問題（type:"written"）を混ぜて${count}問つくってください。`,
    }[format];

    const prompt = `あなたは${m.subject}の先生です。添付した授業プリントの内容に忠実な練習問題を${count}問つくってください。
難易度は「${difficultyText}」にそろえてください。

${formatText}

【プリントへの忠実さ（最優先）】
- まず、プリントに書かれている本文・用語・数値・例題・練習問題を正確に読み取ってください。
- 出題は、プリントに実際に載っている内容（単元・用語・題材・解き方）だけから行ってください。プリントに出てこない単元や発展的な知識は使わないでください。
- プリントに問題が載っている場合は、その問題と同じ形式・同じ解き方の類題（数値や語句を少し変えたもの）を優先して作ってください。
- プリントの構成（穴埋め・用語の説明・計算・読解など）に合わせた問い方にしてください。
- title には、プリントから読み取れる単元名やタイトルを入れてください。
...
- プリントの図そのものを前提にした問題は出さず、条件を文章にした自己完結した問題に作り変えてください。
